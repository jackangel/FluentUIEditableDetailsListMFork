...
import {
    DetailsList,
    IDetailsListProps,
} from 'office-ui-fabric-react/lib/components/DetailsList/DetailsList';
import {
    CommandBar,
...
const EditableGrid = (props: Props) => {
    ...
    const dismissPanelForAdd = React.useCallback(
    ...
    >(undefined);
    const [announced, setAnnounced] = React.useState<JSX.Element | undefined>(
        undefined
    );
    const [isUpdateColumnClicked, setIsUpdateColumnClicked] =
        React.useState(false);
    const [isColumnFilterClicked, setIsColumnFilterClicked] =
        React.useState(false);
    ...
    const onSearchHandler = (event: any) => {
        if (event && event.target) {
            ...
            if (queryText) {
                ...
                gridDataTmp.map(
                    (item) => (item._is_filtered_in_grid_search_ = true)
                );
        ...
        const CheckOnUpdate = async () => {
            if (
                defaultGridData.filter(
                    (x) => x._grid_row_operation_ != Operation.None
                ).length > 0
            ) {
                await onGridUpdate();
            }
            ...
            activateCellEdit.forEach((item, index) => {
                ...
                objectKeys
                    .filter(
                        (key) =>
                            key != '_grid_row_id_' &&
                            key != '_grid_row_operation_'
                    )
                    .forEach((objKey) => {
                        gridEditStatus =
                            gridEditStatus ||
                            item['properties'][objKey]['activated'];
                        if (gridEditStatus) {
                            throw BreakException;
                        }
                    });
    ...
    const AddRowsToGrid = (): void => {
        const updateItemName = (): void => {
            if (SpinRef && SpinRef.current.value) {
                setDialogContent(undefined);
                setAnnounced(
                    <Announced message="Rows Added" aria-live="assertive" />
                );
    ...
    const SaveSingleCellValue = (
        ...
        defaultGridDataTmp[internalRowNumDefaultGrid][key] =
            activateCellEdit[internalRowNumActivateGrid]['properties'][key][
                'value'
            ];
        if (
    ...
    const ShowRowEditMode = (
        ...
        if (enableTextField) {
            ...
            setCancellableRows(
                cancellableRows.filter(
                    (row) => row._grid_row_id_ != item._grid_row_id_
                )
            );
        }
    ...
    const RevertRowEditValues = (
        ...
        var baseRow = cancellableRows.filter(
            (x) => x._grid_row_id_ == rowNum
        )[0];
    ...
    const ShowGridEditMode = (): void => {
        ...
        if (props.onGridEditCallback) {
            props.onGridEditCallback([newEditModeValue, activateCellEditTmp]);
        }
    };
    ...
    const HandleRowCopy = (rowNum: number): void => {
        navigator.clipboard
            .writeText(
                ConvertObjectToText(defaultGridData[rowNum], props.columns)
            )
            .then(
                function () {
                    if (props.onGridStatusMessageCallback)
    ...
    const onFilterTagListChanged = React.useCallback(
        (tagList: ITag[] | undefined): void => {
            ...
            filteredColumnsTmp = props.columns.filter(
                (item) =>
                    tagList!.filter((val) => val.key == item.key).length > 0
            );
    ...
    const UpdateColumnFilterValues = (filter: IFilterListProps): void => {
        var gridColumnFilter: IGridColumnFilter =
            getColumnFiltersRefForColumnKey(filter.columnKey);
    ...
    const ShowFilterForColumn = (column: IColumn, index: number): void => {
        ...
        filters
            .filter(
                (item) =>
                    item.index != filter.index &&
                    item.column.key != filter.column.key
            )
    ...
    const GetUniqueColumnValues = (
        ...
        if (!prevFilters || prevFilters.length == 0) {
            ...
            filterItemArr = uniqueVals.map((item) => {
                var filters: IFilterItem[] = prevFilters.filter(
                    (i) => i.text == item
                );
                return {
                    text: item,
    ...
    const getColumnFiltersRefForColumnKey = (
        key: string
    ): IGridColumnFilter => {
        ...
        props.columns.forEach((column, index) => {
            ...
            columnConfigs.push({
                ...
                isFiltered:
                    ...
                    getColumnFiltersRef().filter(
                        (i) => i.column.key == column.key
                    ).length > 0 &&
                    getColumnFiltersRef().filter(
                        (i) => i.column.key == column.key
                    )[0].isApplied
                    ...
                    : (item, rowNum) => {
                          ...
                          switch (column.inputType) {
                            ...
                            case EditControlType.Checkbox:
                                    return (
                                        <span>
                                        <Checkbox defaultChecked={item[column.key]===1}
                                        onChange={(ev, checked)=>{onCheckboxChange(item[column.key], rowNum!,column)}}
                                        disabled={
                                                      !props.enableDefaultEditMode &&
                                                      !editMode &&
                                                      !(
                    ...
                    items: [
                        {
                            ...
                            iconProps: { iconName: 'Filter' },
                            onClick: () =>
                                RowSelectOperations(EditType.ColumnFilter, {}),
                        },
                        {
                            key: 'clearFilters',
                            text: 'Clear Filters',
        ...
        return (
            <div className={controlClass.plainCard}>
                {React.cloneElement(
                    ...
                    { column: column, rowNum: rowNum, rowData: rowData }
                )}
            </div>
        );
    };
        ...
        return (
            ...
            >
                {item && item[column.key]
                    ? column.dateFormater ? column.dateFormater(new Date(item[column.key])) :new Date(item[column.key]).toDateString()
                    : null}
            </span>
        );
    };
                    ...
                    <MarqueeSelection selection={_selection}>
                        <DetailsList
                                    ...
                                    ? defaultGridData.filter(
                                          (x) =>
                                              x._grid_row_operation_ !=
                                                  Operation.Delete &&
                                              x._is_filtered_in_ == true &&
                                              x._is_filtered_in_grid_search_ ==
                                                  true &&
                                              x._is_filtered_in_column_filter_ ==
                                                  true
                                      )
                                    : []
                            }
                            ...
                            shouldApplyApplicationRole={
                                props.shouldApplyApplicationRole
                            }
                            styles={props.styles}
